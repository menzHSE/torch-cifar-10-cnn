...
def train(device, batch_size, num_epochs, learning_rate, cifar_version, finetune_model):
    ...
    # get the data
    train_loader, test_loader, _ = dataset.cifar(
        batch_size, custom_transforms, cifar_version
    )
    ...
    model_stats = summary(
        cnn,
        input_size=(1, 3, model_input_size, model_input_size),
    ...
    trainer_obj = trainer.Trainer(
        cnn, loss_fn, optimizer, device, fname_save_every_epoch, log_level=logging.INFO
    )
